...
if DEVICE == "cuda":
    MODEL_PATH = r"./models/PhoWhisper-small-ct2-fasterWhisper"  # GPU rảnh tay -> model to hơn, WER tốt hơn
    COMPUTE_TYPE = "float16"  # có thể thử "int8_float16" nếu muốn nhanh hơn, chấp nhận giảm nhẹ độ chính xác
    BEAM_SIZE = 5  # GPU: giữ beam cao lấy accuracy vì chi phí không đáng kể
    CPU_THREADS = 0  # không có tác dụng khi DEVICE=cuda
    NUM_WORKERS = 2  # cho phép pipeline nhiều request qua CUDA streams khi có nhiều client
else:
    MODEL_PATH = r"./models/PhoWhisper-small-ct2-fasterWhisper"
    COMPUTE_TYPE = "int8"  # tối ưu nhất cho CPU (CTranslate2 dùng VNNI/AVX512 nếu CPU hỗ trợ)
    BEAM_SIZE = 1  # CPU: beam search đắt -> greedy (beam=1) cho domain hẹp (ride-booking)
    try:
        import psutil

        CPU_THREADS = psutil.cpu_count(logical=False) or 4  # số CORE VẬT LÝ, không phải logical/hyperthread
    except ImportError:
        CPU_THREADS = max(1, (os.cpu_count() or 4) // 2)  # fallback thô nếu chưa cài psutil
...
VAD_FILTER = False  # audio đã được trim silence ở preprocess_audio() -> tắt để tránh chạy Silero VAD 2 lần
CHUNK_LENGTH = 1
LANGUAGE = "vi"
